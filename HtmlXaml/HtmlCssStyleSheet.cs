...
namespace HTMLXaml
{
    ...
    static class HtmlCssParser
    {
        ...
        private static readonly string[] _listStyleTypes = { "disc", "circle", "square", "decimal", "lower-roman", "upper-roman", "lower-alpha", "upper-alpha", "none" };
    ...
    class CssStylesheet
    {
        ...
        {
            int commentStart = text.IndexOf("/*", StringComparison.CurrentCulture);
            if (commentStart < 0)
            {
                return text;
            }

            int commentEnd = text.IndexOf("*/", commentStart + 2, StringComparison.CurrentCulture);
            if (commentEnd < 0)
            {
        ...
        public string GetStyle(string elementName, List<XmlElement> sourceContext)
        {
            ...
            bool checkAssert = elementName == sourceContext[sourceContext.Count - 1].LocalName;
            if (checkAssert == false)
            {
            }
            Debug.Assert(checkAssert == true);
        ...
        List<StyleDefinition> _styleDefinitions;
    }
}
